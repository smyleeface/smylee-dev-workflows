...
class FunctionSlackDispatcher:
    ...
    def __init__(
        ...
        self._api_path_part_command = "dispatcher"
    ...
    def get_function_definition(self, function_role) -> awslambda.Function:
        return awslambda.Function(
            self._app_name,
            Handler="dev_workflow.slack_dispatcher.handler.lambda_handler",
    ...
    def get_function_api_gateway_resource(self) -> apigateway.Resource:
        return apigateway.Resource(
            ...
            PathPart=self._api_path_part_base,
            RestApiId=Ref(self._api_gateway_rest_api),
        )

    def get_function_api_gateway_resource_path(self, parent_api_gateway) -> apigateway.Resource:
        return apigateway.Resource(
            self._app_name + "ApiGatewayResourcePath",
            ParentId=GetAtt(parent_api_gateway, "ResourceId"),
            PathPart=self._api_path_part_command,
            RestApiId=Ref(self._api_gateway_rest_api),
        )
...
"context" : {
    ...
    def add_resource(self, current_template: Template) -> Template:
        ...
        current_template.add_resource(api_gateway_resource_path)
